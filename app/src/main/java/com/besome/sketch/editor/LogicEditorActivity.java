...
public class LogicEditorActivity extends BaseAppCompatActivity implements View.OnClickListener, Vs, View.OnTouchListener, MoreblockImporterDialog.CallBack {
    ...
    for (BlockBean next : eventBlocks) {
        ...
        Rs block = b(next);
        blockIdsAndBlocks.put((Integer) block.getTag(), block);
        o.g = Math.max(o.g, (Integer) block.getTag() + 1);
        o.a(block, 0, 0);
    ...
    for (BlockBean next : eventBlocks) {
        Rs block = blockIdsAndBlocks.get(next.id);
        if (block != null) {
            Rs subStack1RootBlock = blockIdsAndBlocks.get(next.subStack1);
            if (next.subStack1 >= 0 && subStack1RootBlock != null) {
                block.e(subStack1RootBlock);
            }

            Rs subStack2RootBlock = blockIdsAndBlocks.get(next.subStack2);
            if (next.subStack2 >= 0 && subStack2RootBlock != null) {
                block.f(subStack2RootBlock);
            }

            Rs nextBlock = blockIdsAndBlocks.get(next.nextBlock);
            if (next.nextBlock >= 0 && nextBlock != null) {
                block.b(nextBlock);
            }

            for (int i = 0; i < next.parameters.size(); i++) {
...
private void redo() {
    if (!u) {
        ...
        if (historyBlockBean != null) {
            ...
            switch (actionType) {
                case HistoryBlockBean.ACTION_TYPE_ADD:
                    ...
                    break;
                case HistoryBlockBean.ACTION_TYPE_UPDATE:
                    a(historyBlockBean.getCurrentUpdateData(), true);
                    break;
                case HistoryBlockBean.ACTION_TYPE_REMOVE:
                    ...
                    break;
                case HistoryBlockBean.ACTION_TYPE_MOVE:
                    for (BlockBean afterMoveData : historyBlockBean.getAfterMoveData()) {
                        o.a(afterMoveData, true);
                    }
                    int[] locationOnScreenMove = new int[2];
                    o.getLocationOnScreen(locationOnScreenMove);
                    a(historyBlockBean.getAfterMoveData(), historyBlockBean.getCurrentX() + locationOnScreenMove[0], historyBlockBean.getCurrentY() + locationOnScreenMove[1], true);
                    if (historyBlockBean.getCurrentParentData() != null) {
                        a(historyBlockBean.getCurrentParentData(), true);
                    }
